...
1. En el siguiente árbol se han actualizado todas las cotas:

%    \begin{figure}[b!]
    \begin{center}
    \begin{tikzpicture}[->,>=stealth',level/.style={sibling distance = 5cm/#1,
      level distance = 1.5cm}] 
    \node [arn_n, label=right: \tiny{(3.571, 3.429)}, label=above right: \textbf{\tiny{428}},label=below right: \textbf{\tiny{420}}] {\tiny{$P_{0}$}}
        child{ node [arn_n, label=right: \tiny{(3, 4)}, label=below left: \textbf{\tiny{}}, label=above left: \textbf{\tiny{420}}, label=below left: \textbf{\tiny{420}}] {\tiny{$P_{1}$} }
            edge from parent node[above left] {$x_1 \leq 3$}
        	}
        child{ node [arn_n, label=right: \tiny{(4, 2.4)}, label=below right: \textbf{\tiny{410}}, label=above right: \textbf{\tiny{428}}] {\tiny{$P_{2}$}} 
            child{ node [arn_n, label=above left:\textbf{\tiny{423.3}}, label=right: \tiny{(4.1, 2)}, label=below left: \textbf{\tiny{410}}] {\tiny{$P_{3}$}}
...
\item Explicar qué nodos y por qué se pueden podar, si es que se pueden podar.
    ...
    Los nodos $P_1$, $P_5$ se pueden podar por optimalidad (la solución óptima de su RL es entera). 

  \item Indicar cuál sería un siguiente paso aplicando el algoritmo de Dakin.

Aún están sin explorar los nodos $P_4$ y $P_6$; se podría estudiar (resolver) cualquiera de los dos.
\end{enumerate}   